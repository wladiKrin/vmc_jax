...
parser.add_argument('-g', '--trvField', type=int, 
                    default=1, 
...
L = args.lattice
gs = -3.04438 * jnp.array([0.1, 1, 2])
times = jnp.array([3, 2, 6])
g = gs[args.trvField]
tmax = times[args.tmax]
h = 0.0
...
param_name = "ResNetLog_wvSamp_squareLattice_PBC_L="+str(L)+ "_g="+str(g)+ "_num_hidden="+str(num_hidden)+ "_filter_size="+str(filter_size)+ "_numSamples="+str(numSamples)+ "_exactRenorm="+str(exactRenorm) +"_integratorTol="+str(integratorTol)+ "_invCutoff="+str(invCutoff)+ "_tmax="+str(tmax)
...
for x in [3,4]:
    for y in [3,4]:
        observables["XCenter"].add(op.scal_opstr(1. / 4, (op.Sx(xy_to_id(x,y,L)), )))
        observables["ZCenter"].add(op.scal_opstr(1. / 4, (op.Sz(xy_to_id(x,y,L)), )))
...
psi2GSSampler = jVMC.sampler.MCSampler(psi, sample_shape, random.PRNGKey(4321), updateProposer=jVMC.sampler.propose_spin_flip_Z2,
...
psi2Sampler = jVMC.sampler.MCSampler(psi, sample_shape, random.PRNGKey(4321), updateProposer=jVMC.sampler.propose_spin_flip_Z2,
                                 numChains=25, sweepSteps=L*L,
...
jVMC.util.ground_state_search(psi, H_GS, tdvpEquation, psi2Sampler, numSteps=50)
...
data.append([t, 
    ...
    obs["Z"]["MC_error"][0], 
    obs["ZCenter"]["mean"][0],
    obs["ZCenter"]["variance"][0], 
    obs["ZCenter"]["MC_error"][0], 
    obs["ZZ1"]["mean"][0],
    obs["ZZ1"]["variance"][0], 
    obs["ZZ1"]["MC_error"][0], 
    # obs["ZZ2"]["mean"][0],
    ...
    obs["X"]["MC_error"][0], 
    obs["XCenter"]["mean"][0],
    obs["XCenter"]["variance"][0], 
    obs["XCenter"]["MC_error"][0], 
    obs["XX1"]["mean"][0],
    obs["XX1"]["variance"][0], 
    obs["XX1"]["MC_error"][0], 
    # obs["XX2"]["mean"][0],
...
while t < tmax:
    ...
    with h5py.File("../data/data_"+param_name+".h5", 'w') as f:
        ...
        f.create_dataset("zCPol",      data=npdata[:,7])
        f.create_dataset("zCPol_var",  data=npdata[:,8])
        f.create_dataset("zCPol_MC",   data=npdata[:,9])
        f.create_dataset("zz1",        data=npdata[:,10])
        f.create_dataset("zz1_var",    data=npdata[:,11])
        f.create_dataset("zz1_MC",     data=npdata[:,12])
        ...
        f.create_dataset("xCPol",      data=npdata[:,16])
        f.create_dataset("xCPol_var",  data=npdata[:,17])
        f.create_dataset("xCPol_MC",   data=npdata[:,18])
        f.create_dataset("xx1",        data=npdata[:,19])
        f.create_dataset("xx1_var",    data=npdata[:,20])
        f.create_dataset("xx1_MC",     data=npdata[:,21])
