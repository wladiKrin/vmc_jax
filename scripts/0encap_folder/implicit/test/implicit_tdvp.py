...
class TDVP:
    ...
    def solve(self, Eloc, gradients, params):
        ...
        def f(theta):
            sx_f = jnp.imag(jnp.dot(self.S, theta) - F)
            return jnp.linalg.norm(sx_f)
        ...
        solver = jaxopt.LBFGS(fun=f, tol=1e-6, maxiter=5e4)
        update, state = solver.run(params)

        print("func val: ",  state.value)
        print("iter num: ",  state.iter_num)
        print("func val2: ", f(update))
        ...
        print("implicit:", np.array(update)[:5])
        print("explicit:", np.array(update_expl)[:5])
        # print("comp to explicit: ", np.sum(np.abs(update_expl/update_np.x-1)**2))

        print("func valexplicit: ", f(update_expl))
        print("residual expl: ", residual)
    ...
    def __call__(self, netParameters, t, *, psi, hamiltonian, **rhsArgs):
        ...
        update, solverResidual, pinvCutoff = self.solve(Eloc, sampleGradients, tmpParameters)
